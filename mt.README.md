...
For single-file movie/clip targets, normalize and promote receive the staging **kind directory** (not a specific file path), because the orchestrator doesn't know the exact output filename (`{stem}.norm.mp4`) until handbrake has run. 1. handbrake_mp4
